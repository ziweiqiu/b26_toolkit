...
class ConfocalScan(Script):
    ...
    _DEFAULT_SETTINGS = [
        ...
        Parameter('num_points',
                  [Parameter('x', 100, int, 'number of x points to scan'),
                   Parameter('y', 100, int, 'number of y points to scan'),
                   Parameter('z', 50, int, 'number of z points to scan')
                   ]),
        ...
        Parameter('DAQ_channels',
                   [Parameter('x_ao_channel', 'ao0', ['ao0', 'ao1', 'ao2', 'ao3'], 'Daq channel used for x voltage analog output'),
                    ...
                    Parameter('z_ao_channel', 'ao2', ['ao0', 'ao1', 'ao2', 'ao3'], 'Daq channel used for y voltage analog output'),
    ...
    def _function(self):
        ...
        def scan2D():
            ...
            for var2Num in xrange(0, len(self.var2_array)):
        ...
        def scanZ():
            ...
            for var1Num in xrange(0, len(self.var1_array)):
                if self._abort:
                    break
                # initialize APD thread
        ...
        # Return voltages to initial values:
        self.daq_out.set_analog_voltages(
